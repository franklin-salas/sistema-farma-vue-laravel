...
<div style="display: contents;">
    ...
    <form method="post" action="#" autocomplete="off" @submit.prevent="store_object()">
    <div class="row">
        <div class="col-lg-12">
            <div class="">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div v-for="lote in detalle_producto.lotes" :key="lote.id">
                                                <ul v-if="lote.detalle_salidas.length>0 && verifiLote(lote)" class="nav metismenu">
                                                    <li>
                                                        ...
                                                        <ul >
                                                            ...
                                                            <li> Cantidad: {{sumCantida(lote)}}<!--{{lote.detalle_salidas[0].pivot.cantidad}}--> </li>
                                                        </ul>
                                                    </li>
                                                </ul>
                                            </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Salida de producto {{this.producto_agregar.nombre}}</h4>
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-12 col-lg-3 col-xl-3">
                            <div class="form-group">
                                <label class="col-form-label" for="codigo_lote">Lote</label>
                                <v-select 
                                    class="style-chooser seccion-calendario-input-select " 
                                    v-model="lote_select" 
                                    :clearable="false"  
                                    placeholder="Codigo Lote.." 
                                    label="codigo" 
                                    :filterable="true"
                                    :options="arrayLotes" 
...
export default {
  ...
  methods: {
    ...
    sumCantida(lote){
        let n=0;
        for (let index = 0; index < lote.detalle_salidas.length; index++) {
            if (this.nota.id === lote.detalle_salidas[index].salida_id) {
                n += parseFloat(lote.detalle_salidas[index].pivot.cantidad);
            }  
        }
        return n;
    },
    ...
    abrir_add_lote(product){
        ...
        for (let i = 0; i < product.lotes.length; i++) {
            for (let index = 0; index < product.lotes[i].detalle_salidas.length; index++) {
                 if (product.pivot.id === product.lotes[i].detalle_salidas[index].pivot.detalle_salida_id) {
                    this.cantidad_productos += parseFloat(product.lotes[i].detalle_salidas[index].pivot.cantidad);
                }    
            } 
        }   
    ...
    add_lote(){
        if (this.verifi_agragar_lote()) {
            ...
            this.form.lotes.push({
                ...
                'codigo': this.lote_select.codigo,
    ...
    verifi_agragar_lote(){
        if (this.producto_agregar!='' && this.cantidad_lote!='' && this.lote_select!='') {
            return true;
        }
        return false;
    },
    ...
    mostrar_agregar(product, cantidad){
        ...
        for (let i = 0; i < product.lotes.length; i++) {
            for (let index = 0; index < product.lotes[i].detalle_salidas.length; index++) {
                if (this.nota.id === product.lotes[i].detalle_salidas[index].salida_id) {
                    can += parseFloat(product.lotes[i].detalle_salidas[index].pivot.cantidad);
                }    
            } 
        } 
        if (can === cantidad) {
    ...
    if (this.verifi_store_object()) {
      ...
      me.form
        ...
        this.message='Las summas de las cantidades no debe ser mayor a '+this.producto_agregar.pivot.cantidad;
    }
